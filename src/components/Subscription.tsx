            ...
            return (
              <div key={p.id} className={`card p-5 flex flex-col ${p.highlight ? 'border-coral-400 shadow-glow' : ''}`}>
                ...
                <button
                  ...
                  className={`mt-4 w-full rounded-full px-3 py-2.5 text-sm font-semibold transition ${current ? 'bg-slate-100 dark:bg-white/5 text-slate-400 dark:text-white/40 cursor-not-allowed' : p.highlight ? 'btn-primary' : 'btn-ghost'}`}
                >
